...
const Navbar = () => {
  ...
  return (
    ...
    <nav className="container mx-auto flex items-center justify-between py-4 px-6">
        ...
        <div className="flex items-center space-x-4">
            ...
            <button className="relative hover:text-black cursor-pointer">
                <HiOutlineShoppingBag onClick={toggleCartDrawer} className="h-7 w-7 text-gray-700" />
                {cartItemCount > 0 && (
                <span className="absolute -top-1 bg-red-700 text-white text-sm rounded-full px-2 py-0.5">
                   {cartItemCount}
                </span>
                )}
    ...
    <div className="flex jsutify-end p-4">
    ...
    <div className="p-4">
        ...
        <nav className="space-y-4">
           <div>
             <Link to="/collections/all?gender=Men" onClick={toggleNavDrawer} className="black text-gray-600 hover:text-black">
            Men
            </Link>
           </div>
           <div>
             <Link to="/collections/all?gender=Women" onClick={toggleNavDrawer} className="black text-gray-600 hover:text-black">
            Woman
            </Link>
           </div>
            <div>
                <Link to="/collections/all?category=Top Wear" onClick={toggleNavDrawer} className="black text-gray-600 hover:text-black">
            Top Wear
            </Link>
            </div>
            <div>
                <Link to="/collections/all?category=Bottom Wear" onClick={toggleNavDrawer} className="black text-gray-600 hover:text-black">
            Bottom Wear
            </Link>
            </div>
        </nav>
